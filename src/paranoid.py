...
_stamps_lens = (10, 17, 19, 20, 23, 24, 28)

# to check python version and modify for the code to run in python2 versions
if sys.version_info[0] < 3:
    from io import open
    from src.timestamp_validator_27 import validate_timestamp
else:
    from src.timestamp_validator_3 import validate_timestamp
...
def validate_duration(word):
    if re.match(r'^\-?P(?P<days>\dD)?T(?P<hours>\d{1,2}H)?(?P<mins>\d{1,2}M)?', word):
        return True
    return False

def maskGenerator(word, flag=False, is_json=False, maskdates=True):
    ...
    # Validate for a Duration attribute
    if word.startswith('="P') or word.startswith('="-P'):
        if validate_duration(word[2:-1]):
            return word

    # Consider leading and trailing quotes
    if len(word) - 3 in _stamps_lens and word.startswith('="') and word.endswith('"'):
        r, f = validate_timestamp(word[2:-1], maskdates)
...
def xmlTagMask(s, maskdates=True):
    ...
    if ("=" in s) and ("\"" in s):
        ...
        while i < len(s):
            ...
            if s[i] == "=" and '"' in s[i + 1]:
                ...
                y = y + maskGenerator(_cur_word, maskdates=maskdates)
                i += len(_cur_word)
                continue
            y = y + s[i]
            i += 1
...
def xmlSpecificTagOrAttributeMask(s, masked, tag_mask_flag, user_input=None, xpath="", maskdates=True):
    ...
    if "<?" in s:
        ...
        for i in user_input:
            ...
            if not attr and (xpath.endswith(i[1:]) and i.startswith('//') or xpath == i):
                # re.match('.*' + i[1:] + '$' if i.startswith('//') else i, xpath):
                if tag_mask_flag:
                    masked += 1
                return xmlTagMask(s), True, xpath, masked
            elif xpath.endswith(tags[1:]) and tags.startswith('//') or xpath == tags:
                # re.match('.*' + tags[1:] + '$' if tags.startswith('//') else tags, xpath):
                ...
                return s.replace(d[attr], maskGenerator(d[attr]), flag=1), True, xpath, masked + 1
...
def XmlHandleContent(file_content, _cdata_indexes, out, tag_count, masked,
                     tag_mask_flag, list_mask, xpath, should_mask, maskdates):
    _counter = 0
    _start_index = 0
...
def xmlParse(input_file, out, buffer_size, tag_count,
             masked, mask="", maskdates=True, additional=None):
                    ...
                    for symbol_i in range(len(additional)):
                        ...
                        if delta_tag_count == 0:
                            ...
                            tag_count, masked, xpath = XmlHandleContent(
                                file_content, _cdata_indexes, out, tag_count, masked,
                                tag_mask_flag,
                                list_mask, xpath, should_mask, maskdates)
                            file_content = ""
                            _cdata_indexes = list()
                            breakFlag = True
                            break
        ...
        for i in lines:  # iterating through the lines
            ...
            if i_strip.startswith('<') and i_strip[1] != "/":
                i, should_mask, xpath, masked = xmlSpecificTagOrAttributeMask(
                    i, masked, tag_mask_flag, list_mask, xpath, maskdates)  # masking xml attribute
                ...
                if sys.version_info[0] < 3:
                    if should_mask:
                        file2.write(
                            maskGenerator(b[0], flag=True, maskdates=maskdates).decode('utf-8'))  # masking the attributes
                    ...
                    if should_mask:
                        file2.write(maskGenerator(b[0], flag=True, maskdates=maskdates))  # masking the attributes
                    else:
                        file2.write(b[0])
                    if len(b) > 1:
                        xpath, tag_count = xpathModification(xpath, xmlTagMask(b[1]), tag_count)
...
def XmlHandleCdata(file_content, cdata_content, generator, out, tag_count, delta_tag_count, should_mask, masked,
    ...
    if sys.version_info[0] < 3:  # since below python 3 we do not have unicode characters by default.
        if should_mask:
            tempFile_Cdata.write(maskGenerator(cdata_content, flag=True).decode('utf-8'))  # masking the attributes
        ...
        if should_mask:
            tempFile_Cdata.write(maskGenerator(cdata_content, flag=True))  # masking the attributes
                ...
                try:
                    if ext == ".xml":
                        tag_count, tag_mask = xmlParse(names, output, input_args.byteSize, tag_count, tag_mask,
                                                       input_args.mask, bool(input_args.maskdates))
                    ...
                    logger.error("Error in file: " + name, exc_info=e)
...
def main():
    ...
    parser.add_argument("-b", help="Provide byte size to buffer", dest="byteSize", type=int, default="512000000")
    ...
    parser.add_argument("-D", help="Mask dates or preserve as they are, 0 or 1", dest="maskdates", type=int, default="1")
    parser.add_argument("-v", "--version", action='version', version='Version: %s' % version)
